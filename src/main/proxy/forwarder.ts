...
3. Do NOT output any other text before or after the [function_calls] block`
  ...
  private async doForward(
    ...
    // Check if it is a Doubao provider, use dedicated adapter
    if (DoubaoAdapter.isDoubaoProvider(provider)) {
      return this.forwardDoubao(request, account, provider, actualModel, startTime)
    }
  ...
  /**
   * Z.ai Dedicated Forward (Open WebUI v0.6.2 API)
   */
  private async forwardZai(
    ...
    try {
      ...
      // Open WebUI API is stateless - no chat session to manage
      const handler = new ZaiStreamHandler(actualModel)
      handler.setChatId(chatId)
   ...
   * Directly calls www.doubao.com internal API
   */
  private async forwardDoubao(
    ...
    try {
      const adapter = new DoubaoAdapter(provider, account)
      ...
      if (request.stream === true) {
        // For streaming, conversationId might not be known until the stream starts.
        // Use lastConversationId from adapter after stream completes.
        const deleteSessionCallback = shouldDeleteSession()
          ? async () => {
              try {
                const convId = adapter.lastConversationId
                if (convId) {
                  await adapter.deleteSession(convId)
                  console.log('[forwardDoubao] Session deleted:', convId)
                }
              } catch (error) {
                console.error('[forwardDoubao] Failed to delete session:', error)
              }
            }
          : undefined

        // Listen for stream end to trigger cleanup
        if (deleteSessionCallback) {
          const rawStream = response as any
          rawStream.once('end', () => {
            deleteSessionCallback()
          })
          rawStream.once('close', () => {
            deleteSessionCallback()
          })
        }
      ...
      // Non-stream: cleanup sync if enabled
      if (shouldDeleteSession() && conversationId) {
        adapter.deleteSession(conversationId).catch((err) => {
          console.error('[forwardDoubao] Failed to delete session:', err)
        })
      }

      this.applyToolCallsToResponse(response, request.model, request.tools)
      ...
      const latency = Date.now() - startTime
      console.error('[forwardDoubao] Error:', error)
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
        latency,
