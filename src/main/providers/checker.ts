...
export class ProviderChecker {
  ...
  static async checkAccountToken(
    ...
    switch (provider.id) {
      ...
      case 'doubao':
        return this.checkDoubaoToken(account.credentials.token)
      default:
  ...
  private static async checkDoubaoToken(sessionId: string): Promise<TokenCheckResult> {
    if (!sessionId) {
      return { valid: false, error: 'Session ID 不能为空' }
    }

    try {
      // Use the proxy adapter's static validateToken which calls www.doubao.com directly
      const { DoubaoAdapter } = await import('../proxy/adapters/doubao')
      const valid = await DoubaoAdapter.validateToken(sessionId)

      if (valid) {
        return { valid: true, userInfo: { name: 'Doubao User' } }
      }
      return { valid: false, error: 'Session ID 已过期或无效，请重新获取' }
    } catch (error) {
      return {
        valid: false,
