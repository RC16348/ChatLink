      ...
      const validateAndComplete = async () => {
        ...
        try {
          ...
          if (providerType === 'minimax') {
            ...
            // Doubao: cookie name is 'sessionid', but credential field is 'token'
            const sessionId = collectedTokens.sessionid
            validationCredentials = { token: sessionId }
            finalCredentials = { token: sessionId }
            console.log('[OAuthManager] Doubao: Mapped sessionid -> token')
          } else {
