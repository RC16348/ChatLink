...
class StoreManager {
   ...
   * Try to read as unencrypted JSON first (migration from old encryption),
   * then backup the corrupted file and create a new one
   */
  private async recoverFromCorruptedData(storagePath: string): Promise<void> {
    const { readFileSync, renameSync, existsSync, writeFileSync } = await import('fs')
    const { join } = await import('path')
    
    const dataPath = join(storagePath, 'data.json')
    
    if (existsSync(dataPath)) {
      // Try to read as plain JSON (data may have been saved unencrypted)
      try {
        const rawContent = readFileSync(dataPath, 'utf-8')
        const parsed = JSON.parse(rawContent)
        if (parsed && typeof parsed === 'object') {
          // Valid JSON data found - backup the original first
          const backupPath = join(storagePath, `data.migrated.${Date.now()}.json`)
          writeFileSync(backupPath, rawContent, 'utf-8')
          console.log('[Store] Found valid unencrypted data, will migrate after store init')
          // Write back as-is since new store won't use encryption
          return
        }
      } catch (jsonError) {
        console.log('[Store] Could not read as plain JSON, trying backup:', jsonError)
      }
      
      // If plain JSON reading failed, backup the corrupted file
  ...
  /**
   * Get Storage Path
   * Storage path: ~/.chatlink/
   */
  private getStoragePath(): string {
    return join(homedir(), '.chatlink')
  }

  /**
   * Get Encryption Key
   * Disabled file-level encryption - credentials are already individually encrypted
   * via safeStorage. File-level encryption via electron-store's encryptionKey has
   * caused data corruption and config loss on app restart due to incompatible key format.
   */
