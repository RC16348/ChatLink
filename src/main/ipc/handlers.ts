...
export async function registerIpcHandlers(mainWindow: BrowserWindow | null): Promise<void> {
  ...
  ipcMain.handle(IpcChannels.CONFIG_UPDATE, async (_, updates: Partial<AppConfig>) => {
    ...
    // Sync auto-start to OS level
    if ('autoStart' in updates) {
      try {
        app.setLoginItemSettings({
          openAtLogin: updates.autoStart as boolean,
        })
        console.log('[Config] Auto-start synced to OS:', updates.autoStart)
      } catch (error) {
        console.error('[Config] Failed to sync auto-start to OS:', error)
      }
    }
  ...
  ipcMain.handle(IpcChannels.ACCOUNTS_CLEAR_CHATS, async (_, accountId: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      // Support qwen-ai, minimax, zai, perplexity, glm, and mimo providers
  ...
  ipcMain.handle(IpcChannels.APP_CHECK_UPDATE, async () => {
    try {
      const response = await axios.get('https://api.github.com/repos/RC16348/ChatLink/releases/latest', {
        headers: {
      ...
      const releaseUrl = data.html_url || 'https://github.com/RC16348/ChatLink/releases'
