...
export function AddProviderDialog({
  ...
  const supportsOAuth = selectedProviderData && ['glm', 'kimi', 'mimo', 'minimax', 'qwen', 'qwen-ai', 'zai', 'perplexity', 'doubao'].includes(selectedProviderData.id)
  ...
  const handleNextStep = () => {
    if (selectedProvider) {
      setStep(2)
      // For OAuth-supported providers, directly trigger OAuth login
      if (supportsOAuth) {
        setTimeout(() => handleOpenOAuthBrowser(), 100)
      }
    }
  }
