        ...
        <div className="relative px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-lg border border-white/30">
              <img src={iconsPng} alt="ChatLink" className="w-8 h-8" />
            </div>
            <div>
              <h1 className="text-base font-bold text-white tracking-tight">ChatLink</h1>
              <div className="flex items-center gap-1.5 mt-0.5">
                {proxyRunning ? (
                  <>
                    <Wifi size={12} className="text-white/90" />
                    <span className="text-xs text-white/90 font-medium">服务运行中</span>
                  </>
                ) : (
                  <>
                    <WifiOff size={12} className="text-white/90" />
                    <span className="text-xs text-white/90 font-medium">服务已停止</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold">API 端点</span>
              </div>
              <button
      ...
      <main className="flex-1 min-h-0 px-4 overflow-y-auto no-drag">
        {isLoading && selectedProviderAccounts.length === 0 ? (
          ...
          <div className="py-10 text-center">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500">暂无账户</p>
          </div>
        ) : (
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate">
                      {account.name || account.email || '未知账户'}
                    </span>
                  </div>
                  <span className={cn(
